...
export function ChurchSchedule({ church, currentWeek, isCurrentMonth, monthName, year }: { church: ChurchUnit, currentWeek: WeekNumber, isCurrentMonth: boolean, monthName: string, year: number }) {
    ...
    // Show ALL schedules for the month, grouped by week
    const allSlots = church.weeklySchedules || [];
    const slotsByWeek = new Map<WeekNumber, typeof allSlots>();
    allSlots.forEach(slot => {
        const week = slot.week as WeekNumber;
        if (!slotsByWeek.has(week)) slotsByWeek.set(week, []);
        slotsByWeek.get(week)!.push(slot);
    });
    const sortedWeeks = Array.from(slotsByWeek.keys()).sort((a, b) => a - b);
        ...
        <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-6 md:p-8">
            ...
            {allSlots.length > 0 ? (
                    ...
                    {sortedWeeks.map(week => (
                        <div key={week}>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                {slotsByWeek.get(week)!.map((slot, idx) => (
                ...
                <div className="flex flex-col items-center justify-center py-12 text-gray-400 bg-white rounded-xl border border-dashed border-gray-200">
                    ...
                    <p className="font-medium text-gray-500">Tidak ada jadwal bulan ini</p>
                </div>
            )}
        </div>
    );
