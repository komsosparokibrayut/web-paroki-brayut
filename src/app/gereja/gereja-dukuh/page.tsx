...
export default async function Gereja6Page() {
    const data = await getJadwalMisa();
    const churchData = data?.churches.find(c => c.id === "gereja-dukuh");

    // We can fetch this based on selected month picker state if that was meant to be interactive, 
    // but the context wasn't explicitly provided, so we'll use current date as a robust default 
    ...
    const isCurrentMonth = true; // defaulting to true since it's today
    return (
        <div className="min-h-screen pb-12">
            <PageHeader
                title="Gereja St. Fransiskus Xaverius Dukuh"
                subtitle="Gereja Wilayah – Paroki Brayut"
                image="/images/carousel/GerejaDukuh.JPG"
                align="left"
            />
